...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for col in range(self.belief.getNumCols()):  # for every column
            for row in range(self.belief.getNumRows()):  # for every row
                # take the current probability of the location of the other car
                current = self.belief.getProb(row, col)
                # converting column indices into locations
                x = util.colToX(col)
                y = util.rowToY(row)  # converting row indicies into locations
                # find the distance between two cars
                distance = math.sqrt(
                    math.pow((agentX - x), 2) + math.pow((agentY - y), 2))
                # compute with the probability density function
                probDensity = util.pdf(distance, Const.SONAR_STD, observedDist)
                # update the probability
                self.belief.setProb(row, col, current*probDensity)
    ...
    def elapseTime(self) -> None:
        ...
        # create a new believe object
        newBelieve = util.Belief(
            self.belief.getNumRows(), self.belief.getNumCols(), 0)
        current = {}  # also create a list to record the current probabilities
        for col in range(newBelieve.getNumCols()):  # for every column
            for row in range(newBelieve.getNumRows()):  # for every row
                current[(row, col)] = self.belief.getProb(
                    row, col)  # record each probability
        for tiles in self.transProb.keys():  # for every key of the transProb dictionary
            # split the pair into the old tile and the new tile
            (oldTile, newTile) = tiles
            # add the value to the newTile
            newBelieve.addProb(newTile[0], newTile[1],
                               current[oldTile]*self.transProb[tiles])
        newBelieve.normalize()  # normalize
        self.belief = newBelieve  # update the version
...
class ParticleFilter(object):
    ...
    #             for each of the |self.NUM_PARTICLES| new particles.//
    #             To be extra clear: these
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        # create a new dictionary to record the new probabilities
        ...
        for location in self.particles.keys():  # for every particle
            ...
            # calculate the distance between particles and the agent
            distance = math.sqrt(
                math.pow((agentX - x), 2) + math.pow((agentY - y), 2))
            # compute with the probability density function
            probDensity = util.pdf(distance, Const.SONAR_STD, observedDist)
            newProb[location] = current * probDensity  # update the probability
        # create a new dictionary to record the newly calculated particles
        newParticles = collections.defaultdict(int)
        for run in range(self.NUM_PARTICLES):  # runs the number of particle times
            Location = util.weightedRandomChoice(
                newProb)  # select an element randomly
    ...
    def elapseTime(self) -> None:
        ...
        for location in self.particles:  # for every particle
            for i in range(self.particles[location]):
                Location = util.weightedRandomChoice(
                    self.transProbDict[location])  # select an element randomly
                # add 1 to the particle being chosen
                newParticles[Location] += 1
        self.particles = newParticles  # update particles
