...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        nArray = np.linspace(lower_bound, upper_bound,
                             num_bins, endpoint=False)[1:]
        return nArray
        # End your code
    ...
    def discretize_value(self, value, bins):
        ...
        """
        # Begin your code
        disValue = np.digitize(value, bins, right=False)
        return disValue
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        """
        # Begin your code
        cartPos = self.discretize_value(observation[0], self.bins[0])
        cartV = self.discretize_value(observation[1], self.bins[1])
        angle = self.discretize_value(observation[2], self.bins[2])
        tipV = self.discretize_value(observation[3], self.bins[3])
        state = (cartPos, cartV, angle, tipV)
        return state
    ...
    def learn(self, state, action, reward, next_state, done, ep, count):
        """
        ...
        if ep == 2999:
            ...
            nextMax = np.max(self.qtable[tuple(next_state)])

        currentS = self.qtable[tuple(state)][action]
        self.qtable[tuple(state)][action] = currentS + \
            self.learning_rate * (reward + self.gamma * nextMax - currentS)
        # End your code

        # You can add some conditions to decide when to save your table
        global Maxre

        if ep == 2999 and count/ep > Maxre:
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        #state = Agent(env).discretize_observation(env.reset())
