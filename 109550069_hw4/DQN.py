...
class Agent():
    ...
    def learn(self, ct):
        ...
        states, actions, rewards, nextStates, done = Memory
        ...
        evaluateQ = self.evaluate_net(States).gather(1, Actions)
        nextQ = self.target_net(NextStates).detach()
        for i in range(len(done)):
            if(done[i]):
                nextQ[i] = 0
        targetQ = Rewards + self.gamma * \
            nextQ.max(1)[0].view(self.batch_size, 1)
        loss = torch.nn.MSELoss()
        error = loss(evaluateQ, targetQ)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            val = torch.unsqueeze(torch.FloatTensor(state), 0)
            randNum = np.random.uniform(0, 1)
            if randNum > self.epsilon:
                Action = self.evaluate_net(val)
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        max_q = torch.max(self.target_net.forward(
            torch.FloatTensor(self.env.reset())))
        return max_q
        # End your code
