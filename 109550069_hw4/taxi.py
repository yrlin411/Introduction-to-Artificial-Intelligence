...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        Table = self.qtable
        Gamma = self.gamma
        # new Q(s,a) = old Q(s,a) + learning rate * (reward + gamma * max estimate Q(s,a) - old Q(s,a))
        Table[state, action] = Table[state, action] + self.learning_rate * \
            (reward + Gamma *
             np.max(Table[next_state]) - Table[state, action])
