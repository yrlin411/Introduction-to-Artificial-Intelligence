...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        for t in range(self.T):
            ...
            print("Chose classifier: \n%s \nwith accuracy: %f and alpha: %f" % (str(clf), len(accuracy) - sum(accuracy), alpha))
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        1) Create a 2D list "faceOrNot" with shape (len(features), len(dataset)), set the value
        to 1 if the corresponding "featureVal" value is smaller than 0, otherwise set the value
        to 0.
        2) Create a list "errors" to record the errors of each feature (for each, use the weight times 
        the difference between "featureVal" value and the label of them, then sum them up).
        3) Find the smallest error, record its value and the index to know which feature we want. 
        4) If the value of "faceOrNot" of the feature is equal to the original label of the image, 
        update the weight (the original weight times the error and divide by 1 minus the error).
        5) Return the error and the best WeakClassifier class with parameter being the feature with
        minimum error. 
        """
        # identify face or not
        faceOrNot = np.zeros((len(features), featureVals.shape[1]))
        for val in range(featureVals.shape[0]):
            for data in range(featureVals.shape[1]):
                if featureVals[val][data] < 0:
                    faceOrNot[val][data] = 1
                else:
                    faceOrNot[val][data] = 0
        # record all errors
        errors = []
        for val in range(featureVals.shape[0]):
            e = 0
            for data in range(featureVals.shape[1]):
                e += (weights[data] * abs(faceOrNot[val][data] - labels[data]))
            errors.append(e)
